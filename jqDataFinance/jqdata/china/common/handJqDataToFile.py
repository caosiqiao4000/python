...
def getDataByCodeToFile(stockCodeMarket, stockName, monthNumBar=800, numBar=100000,
    ...
    # -----------------------------------------------------------------------
    df0 = get_bars(stockCodeMarket, numBar, unit='60m', fields=field_params,
                   include_now=True, fq_ref_date=datenow,
                   end_dt=datenow)
    print(df0[0:1])
    print(df0.tail(1))
...
def getIndexDataByCodeToFile(stockCodeMarket, stockName, path=my_path):
    # panel = get_price(['000001.XSHG', '000819.XSHG', '000300.XSHG'], start_date=start_date_str, end_date=datenow,
    ...
    df0.to_csv(
        path + stockCodeMarket + "_" + stockName + "day.csv")
    #---------------------------------------------------------------------------hour
    df1 = get_price(stockCodeMarket, start_date=start_date_str, end_date=datenow,
